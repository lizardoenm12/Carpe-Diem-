...
import { getPoemaAleatorio, type NivelEmocional, type Poema } from "@/lib/poemas";
...
const getTextoPoema = (nivel: number) => {
  if (nivel === 6) return { texto: "#1a6640", autor: "#2d8a58" };
  if (nivel === 5) return { texto: "#27500A", autor: "#3a7a50" };
  if (nivel === 4) return { texto: "#6b5500", autor: "#8a7030" };
  if (nivel === 3) return { texto: "#7a3010", autor: "#a04020" };
  if (nivel === 2) return { texto: "#7a2020", autor: "#a03030" };
  if (nivel === 1) return { texto: "#6a1818", autor: "#8a2828" };
  return { texto: "#27500A", autor: "#3a7a50" };
};
...
export default function Calendario() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      setCurrentUser(user);
      if (user) {
        cargarEventos(user.uid);
        try {
          const snapC = await getDoc(doc(db, "checkins", `${user.uid}_${new Date().toDateString()}`));
          if (snapC.exists()) {
            const nivel = snapC.data().nivel ?? 5;
            setNivelEmocion(nivel);
            setPoema(getPoemaAleatorio(nivel as NivelEmocional));
          } else {
            setPoema(getPoemaAleatorio(5));
          }
  ...
  const agregarEvento = async () => {
    ...
    try {
      await addDoc(collection(db, "eventos"), {
        titulo: nuevoTitulo, fecha: nuevaFecha, tipo: nuevoTipo,
        uid: currentUser.uid, completado: false,
      });
      setNuevoTitulo(""); setNuevaFecha(""); setMostrarForm(false);
  ...
  const getEventosDelDia = (dia: number) => {
    ...
    return eventos.filter(e => e.fecha === `${year}-${month}-${diaStr}`);
  };
  ...
  const nombreDiaHoy          = hoy.toLocaleDateString("es-ES", { weekday:"long", day:"numeric", month:"long" });
  const coloresPoema          = getTextoPoema(nivelEmocion);

  const CheckItem = ({ evento }: { evento: Evento }) => {
    const c    = colores[evento.tipo];
    const dias = getDias(evento.fecha);
    return (
      <div style={{ display:"flex", alignItems:"flex-start", gap:"10px", padding:"8px 0", borderBottom:`0.5px solid ${borderCard}` }}>
        <div
          onClick={() => eliminarEvento(evento)}
          style={{ width:"16px", height:"16px", borderRadius:"4px", border:`1.5px solid ${evento.completado ? c.punto : borderCard}`, background:evento.completado ? c.punto : bgCard, cursor:"pointer", flexShrink:0, marginTop:"2px", display:"flex", alignItems:"center", justifyContent:"center" }}
        >
          {evento.completado && <span style={{ color:"white", fontSize:"10px", fontWeight:700 }}>✓</span>}
        </div>
        <div style={{ flex:1 }}>
          <p style={{ fontSize:"12px", fontWeight:500, color:"#444", margin:"0 0 2px" }}>{evento.titulo}</p>
          <p style={{ fontSize:"10px", color:"#888", margin:0 }}>
            {dias === 0 ? "Hoy" : dias === 1 ? "Mañana" : `En ${dias} días`} · {c.label}
          </p>
        </div>
      </div>
    );
      ...
      <div className="page-scroll">
        <section style={{ flex:1, padding:"32px", display:"flex", gap:"24px", alignItems:"flex-start" }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ marginBottom:"28px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"4px" }}>
                <div>
                  {/* ── CAMBIO 1: h1 usa colorTitulo del semáforo ── */}
                  <h1 style={{ fontSize:"28px", fontWeight:600, margin:"0 0 4px", color: colorTitulo, lineHeight:1.1 }}>
                    Calendario
                  </h1>
                  {/* ── CAMBIO 2: subtítulo fecha usa colorSub del semáforo ── */}
                  <p style={{ fontSize:"13px", color: colorSub, margin:0, textTransform:"capitalize" }}>
                    {nombreDiaHoy}
                  </p>
                </div>
                <div style={{ display:"flex", gap:"8px", marginTop:"4px" }}>
                  <button
                    ...
                    style={{ padding:"8px 14px", borderRadius:"8px", background:bgCard, border:`0.5px solid ${borderCard}`, fontSize:"12px", cursor:"pointer", color:"#555" }}
                  >
                    {vistaCalendario ? "Vista lista" : "Vista calendario"}
                  </button>
                  <button
                    onClick={() => setMostrarForm(!mostrarForm)}
                    style={{ padding:"8px 16px", borderRadius:"8px", background:"#B2D8B2", border:"none", fontSize:"13px", fontWeight:500, color: colorTitulo, cursor:"pointer" }}
                  >
                    + Agregar evento
                  </button>
                </div>
              ...
              {/* Poema debajo del título */}
              {poema && (
                <div style={{ marginTop:"16px", paddingLeft:"2px" }}>
                  <p style={{
                    fontFamily: "Georgia, serif",
                    fontStyle: "italic",
                    fontSize: "14px",
                    lineHeight: 1.85,
                    color: coloresPoema.texto,
                    margin: "0 0 6px",
                    whiteSpace: "pre-line",
                  }}>
                    {poema.texto}
                  ...
                  <p style={{
                    ...
                    color: coloresPoema.autor,
                    margin: 0,
                  }}>
                    — {poema.autor}, <em>{poema.obra}</em>
                  </p>
                </div>
              )}
            </div>
            ...
            {/* ── Mensajes IA ── */}
            {mensajes.length > 0 && (
              <div style={{ background:bgCard, borderRadius:"16px", padding:"16px 20px", border:`0.5px solid ${borderCard}`, marginBottom:"20px" }}>
            ...
            {mostrarForm && (
              <div style={{ background:bgCard, borderRadius:"16px", padding:"20px", border:`0.5px solid ${borderCard}`, marginBottom:"20px" }}>
                {/* ── CAMBIO 3: "Nuevo evento" usa colorTitulo ── */}
                <p style={{ fontSize:"14px", fontWeight:500, margin:"0 0 14px", color: colorTitulo }}>Nuevo evento</p>
                <input
                  value={nuevoTitulo} onChange={e => setNuevoTitulo(e.target.value)}
                  placeholder="Nombre del evento"
                  style={{ width:"100%", padding:"10px", borderRadius:"8px", border:`0.5px solid ${borderCard}`, fontSize:"13px", marginBottom:"10px", boxSizing:"border-box", outline:"none" }}
                />
                <div style={{ display:"flex", gap:"10px", marginBottom:"10px" }}>
                  <input
                    type="date" value={nuevaFecha} onChange={e => setNuevaFecha(e.target.value)}
                    style={{ flex:1, padding:"10px", borderRadius:"8px", border:`0.5px solid ${borderCard}`, fontSize:"13px", outline:"none" }}
                  />
                  <select
                    value={nuevoTipo} onChange={e => setNuevoTipo(e.target.value as any)}
                    style={{ flex:1, padding:"10px", borderRadius:"8px", border:`0.5px solid ${borderCard}`, fontSize:"13px", outline:"none" }}
                  >
                ...
                {/* ── CAMBIO 4: botón Guardar usa colorTitulo ── */}
                <button
                  onClick={agregarEvento} disabled={!nuevoTitulo||!nuevaFecha||guardando}
                  style={{ width:"100%", padding:"10px", borderRadius:"8px", background:"#B2D8B2", border:"none", fontSize:"13px", fontWeight:500, color: colorTitulo, cursor:"pointer" }}
                >
            ...
            {vistaCalendario && (
              <div style={{ background:bgCard, borderRadius:"16px", padding:"20px", border:`0.5px solid ${borderCard}`, marginBottom:"20px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"16px" }}>
                  <button onClick={() => setMesActual(new Date(mesActual.getFullYear(), mesActual.getMonth()-1))} style={{ padding:"6px 12px", borderRadius:"8px", background:"transparent", border:`0.5px solid ${borderCard}`, cursor:"pointer", fontSize:"16px" }}>‹</button>
                  {/* ── CAMBIO 5: nombre del mes usa colorTitulo ── */}
                  <p style={{ fontSize:"15px", fontWeight:500, margin:0, textTransform:"capitalize", color: colorTitulo }}>{nombreMes}</p>
                  <button onClick={() => setMesActual(new Date(mesActual.getFullYear(), mesActual.getMonth()+1))} style={{ padding:"6px 12px", borderRadius:"8px", background:"transparent", border:`0.5px solid ${borderCard}`, cursor:"pointer", fontSize:"16px" }}>›</button>
                </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"2px" }}>
                  {Array.from({ length:offset }).map((_,i) => <div key={`empty-${i}`} style={{ minHeight:"52px" }}/>)}
                  {Array.from({ length:diasEnMes }).map((_,i) => {
                    ...
                    return (
                      <div key={dia} style={{ minHeight:"52px", padding:"4px", borderRadius:"8px", background:esHoy?"#E1F5EE":"transparent", border:esHoy?`0.5px solid ${borderCard}`:"0.5px solid transparent" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
              {eventosFuturos.filter(e => !e.completado).length === 0 ? (
                <div style={{ background:bgCard, borderRadius:"16px", padding:"24px", border:`0.5px solid ${borderCard}`, textAlign:"center" }}>
                ...
                eventosFuturos.filter(e => !e.completado).map(e => {
                  ...
                  return (
                    <div key={e.id} style={{ background:bgCard, borderRadius:"16px", padding:"16px 20px", border:`0.5px solid ${c.border}`, display:"flex", alignItems:"center", gap:"16px" }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <p style={{ fontSize:"12px", color:"#888", margin:0 }}>
                          {new Date(e.fecha+"T00:00:00").toLocaleDateString("es-ES",{weekday:"long",day:"numeric",month:"long"})}
                        </p>
                      </div>
          ...
          <div style={{ width:"260px", flexShrink:0, position:"sticky", top:"32px" }}>
            <div style={{ background:bgCard, borderRadius:"16px", padding:"20px", border:`0.5px solid ${borderCard}` }}>
              {/* ── CAMBIO 6: títulos del panel lateral usan colorTitulo ── */}
              <p style={{ fontSize:"14px", fontWeight:500, color: colorTitulo, margin:"0 0 2px" }}>Esta semana</p>
              ...
              <div style={{ borderTop:`0.5px solid ${borderCard}`, marginTop:"16px", paddingTop:"16px" }}>
                {/* ── CAMBIO 7: "Próximamente" usa colorTitulo ── */}
                <p style={{ fontSize:"14px", fontWeight:500, color: colorTitulo, margin:"0 0 2px" }}>Próximamente</p>
                ...
                {eventoProximos.length === 0 ? (
                  ...
                  <div>{eventoProximos.map(e => <CheckItem key={e.id} evento={e}/>)}</div>
                )}
              </div>

              <div style={{ borderTop:`0.5px solid ${borderCard}`, marginTop:"16px", paddingTop:"16px" }}>
                {/* ── CAMBIO 8: "Leyenda" usa colorTitulo ── */}
                <p style={{ fontSize:"13px", fontWeight:500, color: colorTitulo, margin:"0 0 10px" }}>Leyenda</p>
                {Object.entries(colores).map(([key,c]) => (
