...
export default function Dashboard() {
  ...
  if (mostrarContenido) {
    return (
      <main style={{display:"flex",minHeight:"100vh",background: nivelEmocion >= 5 ? "#EDFBF3" : nivelEmocion === 4 ? "#FDFBEA" : nivelEmocion === 3 ? "#FEF6ED" : nivelEmocion === 2 ? "#FEF0EE" : nivelEmocion === 1 ? "#FAF0F0" : "#F5F5DC", transition:"background 0.5s"}}>
        <Sidebar />
        <section style={{flex:1,padding:"32px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"16px",marginBottom:"24px"}}>
            {[
              { icon:"📚", titulo:"Apuntes", desc:"Tus notas organizadas" },
              { icon:"📅", titulo:"Calendario", desc:"Tus próximas entregas" },
              { icon:"🎮", titulo:"Juegos", desc:"Repasa sin estrés" },
              { icon:"⏳", titulo:"Racha", desc:"Sigue tu progreso" },
            ].map((item) => (
              <div key={item.titulo} style={{background:"white",borderRadius:"16px",padding:"20px",border:"0.5px solid #e0e0e0",cursor:"pointer"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"10px",width:"100%",maxWidth:"420px"}}>
          {emociones.map((e, i) => (
            <div
              ...
              style={{display:"flex",alignItems:"center",gap:"14px",padding:"14px 16px",borderRadius:"12px",border:`0.5px solid ${e.color}`,background:emocionSeleccionada===i?e.fondo:"white",cursor:"pointer"}}
            >
